...
async function getProviderAvailability(requestedProvider: unknown): Promise<ProviderAvailability> {
	const provider = normalizeProviderInput(requestedProvider ?? loadConfig().provider);
	const skipGeminiWebCheck = provider === "perplexity";
	const geminiWebAvail = skipGeminiWebCheck ? null : await isGeminiWebAvailable();
	return {
...
async function loadCuratorBootstrap(requestedProvider: unknown): Promise<CuratorBootstrap> {
	const availableProviders = await getProviderAvailability(requestedProvider);
	return {
		availableProviders,
